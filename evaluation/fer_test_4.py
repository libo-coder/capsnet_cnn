...
Example use:
    ...
    $ python evaluation/test.py --dataset=fer --network-type=capsnet_single_digit_capsule
    --weights=./weights-01.h5
...
def evaluate(dataset, network_type, weights):
    ...
    if 'capsnet' in network_type:
        if dataset == 'fer' and network_type == 'capsnet_single_digit_capsule':
            from CapsNet_vs_CNN.capsnet.singledigitcapsule.fer_capsulenet_4 import CapsNet
            model, _, _ = CapsNet(input_shape=fer2013_4.IMAGE_SHAPE, n_class=fer2013_4.CLASSES, routings=CAPSNET_ROUTINGS)
        ...
        if dataset == 'fer':
            ...
            model = resnet_v2(input_shape=fer2013_4.IMAGE_SHAPE, depth=110)
    ...
    for rotation in ROTATIONS:
        if 'capsnet' in network_type:
            x_test, y_test = fer2013_4.get_test_data_for_capsnet(rotation)
            ...
            x_test, y_test = fer2013_4.get_test_data_for_cnn(rotation)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default='fer', required=False, type=str)
    parser.add_argument('--network-type', default='capsnet_single_digit_capsule', required=False, type=str)

    parser.add_argument('--weights',
                        default='/home/wh/chen/pycharm/CapsNet_vs_CNN/capsnet/singledigitcapsule/result/fer_singlecaps_4(2)/weights-01.h5',
