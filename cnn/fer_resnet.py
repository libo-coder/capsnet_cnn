...
def resnet_v2(input_shape, depth, num_classes=6):
    ...
    x = keras.layers.AveragePooling2D(pool_size=12)(x)
    y = keras.layers.Flatten()(x)

    outputs = keras.layers.Dense(fer2013.CLASSES,
                                 activation='softmax',
...
if __name__ == '__main__':
    model = resnet_v2(input_shape=fer2013.IMAGE_SHAPE, depth=depth)
    ...
    model.fit_generator(fer2013.get_train_generator_for_cnn(batch_size),
                        steps_per_epoch=fer2013.TRAIN_SIZE / batch_size,
                        validation_data=fer2013.get_validation_data_for_cnn(),
                        epochs=epochs,
                        verbose=1,
                        workers=1,
                        callbacks=callbacks)
    ...
    # Score trained model.
    x_test, y_test = fer2013.get_test_data_for_cnn()
